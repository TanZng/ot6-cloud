...
session = boto3.Session(
    aws_secret_access_key="GLBDicF/7wLg2uYsbX0yUc7t+YfAPg7stoIOfETv",
    aws_access_key_id="AKIAUELZXXSW6R2722JI",
)

# to access the low-level client interface, returns dicts (may need to be parsed)
s3_client = session.client('s3')                                                                                      ')
# s3_client = boto3.client('s3')

# similarly, to access the high-level one, returns already objects :D
...
def set_bucket_encryption(client, bucket_name):
    """
...
def set_bucket_versioning(client, bucket_name):
...
client_bucket_name, client_reponse = create_bucket("boto3-client-bucket",s3_client)
set_bucket_encryption(s3_client, client_bucket_name)
set_bucket_versioning(s3_client, client_bucket_name)

resource_bucket_name, resource_reponse = create_bucket("boto3-resource-bucket",s3_resource)
versioning = s3_resource.BucketVersioning(resource_bucket_name)
# check status
...
# comes in the form of a dictionnary
# print("client bucket", client_reponse)

# comes in a nice bucket object
# print("resource_bucket", resource_reponse)
