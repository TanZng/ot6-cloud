...
def push_file(file_name, s3_client):
    s3_client.upload_file(Filename = file_name, Key = file_name)
...
def get_from_s3(s3_client, bucket_name, file_name, sql_expression):
    ...
    file1.close() 
    return pd.read_csv("myfile.txt", header=None)
...
def main():
    ...
    push_file(CSV_RESULTS,s3_client)
